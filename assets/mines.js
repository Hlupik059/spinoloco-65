...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-dcdb840f";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-dcdb840f">'+'<span class="tile-face-dcdb840f tile-front-dcdb840f"></span>'+'<span class="tile-face-dcdb840f tile-back-dcdb840f"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-dcdb840f")){return;}
var back=tile.querySelector(".tile-back-dcdb840f");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-dcdb840f");if(isMine){tile.classList.add("boom-dcdb840f");}
if(dim){tile.classList.add("dimmed-dcdb840f");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-dcdb840f")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-dcdb840f");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-dcdb840f")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-dcdb840f");void panelEl.offsetWidth;panelEl.classList.add("jolt-dcdb840f");}
endRound();}
